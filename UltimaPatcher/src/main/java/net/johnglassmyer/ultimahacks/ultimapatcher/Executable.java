...
class Executable {
	...
	void produceSegmentsAsm() {
		L.info(String.format("; %s (0x%X bytes)", path.getFileName(), fileLength));
		String date = LocalDateTime.now().format(DateTimeFormatter.BASIC_ISO_DATE);
		L.info(String.format("; segments.asm generated by UltimaPatcher on %s", date));
		L.info("; https://github.com/JohnGlassmyer/UltimaHacks");
		range(0, segments.size()).forEach(segmentIndex -> {
			Segment segment = segments.get(segmentIndex);

			String segmentLine = String.format(
					"defineSegment %3d, 0x%04X, 0x%04X",
					segmentIndex,
					segmentIndex * 8,
					segment.tableEntry.segmentBase);

			if (segment.tableEntry.getLength() == 0) {
				segmentLine = String.format(";%s ; zero-length segment", segmentLine);
			}
